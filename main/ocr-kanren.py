...
def process_all_images_in_folder(input_folder_path, output_folder_path, kanren_sample_path):
    # 出力フォルダが存在しない場合は作成
    ...
    for filename in os.listdir(input_folder_path):
        ...
        if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
            ...
            # kanren-sample.txt に一文一行で追加
            with open(kanren_sample_path, 'a', encoding='utf-8') as kanren_file:
                for segment in segments:
                    for line in segment.split("。"):  # 一文一行に分割
                        line = line.strip()
                        if line:
                            kanren_file.write(line + "。\n")

            # 一時ファイルを削除
            os.remove(os.path.join(output_folder_path, 'temp.txt'))

# 使用例
input_folder_path = 'takken/R5/R5-takken-kanren'  # 画像が含まれるフォルダのパスを指定
output_folder_path = 'takken/R5/R5-kanren'
kanren_sample_path = 'kanren/kanren-sample.txt'  # kanren-sample.txt のパスを指定
process_all_images_in_folder(input_folder_path, output_folder_path, kanren_sample_path)
