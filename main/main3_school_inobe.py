...
def process_query_file(file_path, chat_bot, output_folder, vector_output_folder, related_output_folder, similarity_output_folder):
    try:
    ...
    # school-kanren フォルダ内のJSONファイルを探索
    kanren_folder = "kanren/title_vector"
    json_files = [f for f in os.listdir(kanren_folder) if f.endswith(".json")]

    if not json_files:
        print(f"類似度を計算するためのJSONファイルが見つかりません: {kanren_folder}")
        return
