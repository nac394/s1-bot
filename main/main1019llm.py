...
query_folder = "takken/R1/R1-takken-query"
# 出力ファイルを保存するフォルダを指定
output_folder = "takken/R1/R1-takken-answer-GPT"
# ベクトル化したクエリを保存するフォルダを指定
vector_output_folder = "takken/R1/R1-takken-query-vectors"
# 関連文を保存するフォルダを指定
related_output_folder = "takken/R1/R1-takken-related-sentences"
...
def process_query_file(file_path, chat_bot):
    ...
    if lines_with_keyword_within_7:
        ...
        topk_per_line = 2  # query_line1 の関連文の数
    ...
    if all_results:
        ...
        if line_similarity_results:
            output_content += "【解説と対応する参考書ページ】\n\n"
            for item in line_similarity_results:

                # 「」で囲まれたテキストを保持（必要に応じて色付け等の処理を追加可能）
        ...
        # 各処理対象行から取得した関連文を「関連文」セクションとして追加
        if line_related_results:
            output_content += "【関連文】\n\n"
            for idx, related_data in enumerate(line_related_results, start=1):
                output_content += f"処理対象行{idx}: {related_data['query_line']}\n"
                for res_idx, result in enumerate(related_data['results'], start=1):
                    output_content += f"  関連文{res_idx}: {result['text']}（類似度: {result['score']}）\n"
                output_content += "\n"

        # ChatGPTの生の回答を「ChatGPTの返答」セクションとして追加
        if response:
